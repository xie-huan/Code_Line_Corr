...
def start_calc_rank(start_dir):
    ...
    for elem in curr_file_list:
        ...
        if os.path.isdir(curr_path):
            ...
            # df            df              df              list
            pearson_corr, spearman_corr, kendall_corr, fault_line_data = get_corr(os.path.dirname(curr_path))
            rank_dict = calc_rank(pearson_corr, spearman_corr, kendall_corr, fault_line_data)
